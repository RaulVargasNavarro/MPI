 ...
 * File:     mpi_histo.c
 *
 * Created by: Mary Thomas (SDSU)
 * 	     Feb, 2014
 *	     Based on Pacheco solution
 * 
 * Purpose:  Use MPI to implement a program that creates a histogram
 *
 * Compile:  mpicc -g -Wall -o mpi_hist  mpi_hist.c
 * Run:      mpiexec -n <comm_sz> ./mpi_histo
 *
...
void Get_input1(int* bin_count_p, float* min_meas_p, float* max_meas_p,
      int* data_count_p, int* local_data_count_p,
      int my_rank, int comm_sz, MPI_Comm comm);
void Get_input2(int* bin_count_p, float* min_meas_p, float* max_meas_p, 
      int* data_count_p, int* local_data_count_p, int argc, char* argv[],
      int my_rank, int comm_sz, MPI_Comm comm);
...
int main(int argc, char* argv[]) {
  ...
  : // from STDIO
   //Get_input1(&bin_count, &min_meas, &max_meas, &data_count, 
   //      &local_data_count, my_rank, comm_sz, comm);
   
   // get user inputs for bin_count, max_meas, min_meas, and data_count
   // from command line
   Get_input2(&bin_count, &min_meas, &max_meas, &data_count, 
         &local_data_count, argc, argv, my_rank, comm_sz, comm);
...
void Find_bins(
   ...
   // each processors sorts its local data
   int i,bin;
   ...
   // sum is performed across all processors, for each array element
...
/*---------------------------------------------------------------------*/
// this routine proccesses input from STDIN 
void Get_input1(int* bin_count_p, float* min_meas_p, float* max_meas_p, 
      int* data_count_p, int* local_data_count_p, 
      int my_rank, int comm_sz, MPI_Comm comm) {
   int local_ok = 1;
   ...
   Check_for_error(local_ok, "Get_input1",
...
/*---------------------------------------------------------------------*/
void Get_input2(int* bin_count_p, float* min_meas_p, float* max_meas_p, 
// this routine proccess command line args 
      int* data_count_p, int* local_data_count_p, int argc, char* argv[], 
      int my_rank, int comm_sz, MPI_Comm comm) {

      printf("Enter: Get_input2 \n");

   if (my_rank == 0) 
      if (argc != 5) Usage(argc, argv[0]);
   ...
   *local_data_count_p = *data_count_p / comm_sz;
   ...
   if((data_count_p > 0) && (*data_count_p % comm_sz != 0)) {
      printf("Error: Get_input2: ");
      printf("Error: data_count[%d] must be evenly divisible by comm_sz[%d]\n",
                     data_count_p, comm_sz);
      MPI_Finalize();
      exit(-1);
   }
...
#  ifdef DEBUG
   printf("P[%d]: bin_count  = %d\n",my_rank, *bin_count_p );
   printf("P[%d]: min_meas   = %f,   max_meas = %f\n", my_rank, *min_meas_p, *max_meas_p);
   printf("P[%d]: data_count = %d\n",my_rank, *data_count_p);
   printf("P[%d]: local_data_count = %d\n",my_rank, local_data_count_p);
#  endif
