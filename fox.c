 ...
 *
 * Modified by Raul Vargas Navarro
 * SDSU CSRC Spring 2014
 * from Pacheco's Chp 7 source code
 * and from
 *
 ...
 * file1 = file name of the first matrix (read only)
 * file2 = file name of the second matrix (read only)
 * file3 = file name of the product matrix (will be overwritten if exists)
 *
 * Notes:  
...
main(int argc, char* argv[]) {
    ...
    if (my_rank == 0) {
      if (argc >4)
      {
   	n=atoi(argv[1]);
        ...
        strcpy(file_name3,argv[4]);
      }
      else
      {
        printf("Usage: %s n file1 file2 file3 ....\n",argv[0]);
	printf("\t n = order of the matrices\n");
	printf("\t file1 = file name of the first matrix (read only)\n");
	printf("\t file2 = file name of the second matrix (read only)\n");
	printf("\t file3 = file name of the product matrix (will be overwritten if exists)\n");
        exit(1);
      }
    }

    MPI_Bcast(&n, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(file_name1,sizeof(file_name1)+1,MPI_CHAR,0,MPI_COMM_WORLD);
    MPI_Bcast(file_name2,sizeof(file_name2)+1,MPI_CHAR,0,MPI_COMM_WORLD);
    MPI_Bcast(file_name3,sizeof(file_name3)+1,MPI_CHAR,0,MPI_COMM_WORLD);
    n_bar = n/grid.q;

    local_A = Local_matrix_allocate(n_bar);
    Order(local_A) = n_bar;
    Read_matrix(file_name1, local_A, &grid, n);

    local_B = Local_matrix_allocate(n_bar);
    Order(local_B) = n_bar;
    Read_matrix(file_name2, local_B, &grid, n);
    ...
    printf("%f\n", raw_time);
    Print_matrix(file_name3, local_C, &grid, n);
...
LOCAL_MATRIX_T* Local_matrix_allocate(int local_order) {
    ...
    if (temp == NULL) printf("Error in line 251\n");
    return temp;
...
void Read_matrix(
    ...
    if (grid->my_rank == 0) {
        ...
        if (temp == NULL) printf("Error en linea 285\n");
        printf("%s\n", file_name);
        fflush(stdout);
...
void Print_matrix(
    ...
    if (grid->my_rank == 0) {
        temp = (float*) malloc(Order(local_A)*sizeof(float));
        if(temp == NULL) printf("Error in line 333\n");
//        printf("%s\n", title);
        for (mat_row = 0;  mat_row < n; mat_row++) {
            grid_row = mat_row/Order(local_A);
            coords[0] = grid_row;
            for (grid_col = 0; grid_col < grid->q; grid_col++) {
                ...
                if (source == 0) {
                    for(mat_col = 0; mat_col < Order(local_A); mat_col++)
                        fprintf(fp,"%f ", Entry(local_A, mat_row, mat_col));
                } else {
                    MPI_Recv(temp, Order(local_A), MPI_FLOAT, source, 0,
                        grid->comm, &status);
                    for(mat_col = 0; mat_col < Order(local_A); mat_col++)
                        fprintf(fp,"%f ", temp[mat_col]);
                }
            }
            fprintf(fp,"\n");
        }
